...
class RegressionLoss:
    ...
    def __init__(
        ...
        self.P_std = P_std
        self.sigma_data = sigma_data
        # land_mask: (1, H, W) float32 tensor, 1=land 0=ocean.
        # Stored on CPU; moved to the correct device on first use.
    ...
    def __call__(self, net, img_clean, img_lr, labels=None, augment_pipe=None):
        ...
        torch.Tensor
            A tensor representing the loss calculated based on the network's
            predictions, zeroed at ocean pixels.
        """
        rnd_normal = torch.randn([img_clean.shape[0], 1, 1, 1], device=img_clean.device)
        sigma = (rnd_normal * self.P_std + self.P_mean).exp()
        weight = (
...
class ResLoss:
    ...
    def __call__(
        ...
        # Apply land mask for full-resolution training only.
        # In the patched path y has been cropped to (B*patch_num, C, pH, pW);
        # the mask would need per-patch slicing which is complex and the Vietnam
        # domain is mostly land anyway, so masking is skipped there.
        if self.land_mask is not None and (
            self.img_shape_x == self.patch_shape_x
            and self.img_shape_y == self.patch_shape_y
        ):
            mask = self.land_mask.to(loss.device)
            loss = loss * mask
